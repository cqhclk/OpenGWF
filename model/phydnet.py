...
def main():
    # 设备配置，选择 GPU 设备 cuda:1
    device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')
    
    # 假设输入和输出的形状
    input_shape = (80, 80)  # 输入图像大小为 64x64
    T_in = 6                # 输入序列长度（例如6小时）
    T_out = 6               # 输出序列长度（例如6小时）
    batch_size = 4          # 批次大小为 4

    # 初始化模型
    model = PhyDNet_Model(in_shape=(65, *input_shape), T_in=T_in, T_out=T_out, device=device).to(device)

    # 创建虚拟数据（模拟训练数据）
    # 假设输入数据维度为 (batch_size, T_in, 1, 64, 64)
    input_tensor = torch.randn(batch_size, T_in, 65, *input_shape).to(device)
    # 假设目标数据维度为 (batch_size, T_out, 1, 64, 64)
    target_tensor = torch.randn(batch_size, T_out, 65, *input_shape).to(device)
    
    # 计算损失和输出
    predictions, loss = model.predict(input_tensor, target_tensor, compute_loss=True)
    ...
    # 如果仅需要预测，可以使用 compute_loss=False
    predictions, _ = model.predict(input_tensor, compute_loss=False)
    print(f"Predictions shape (without loss computation): {predictions.shape}")
